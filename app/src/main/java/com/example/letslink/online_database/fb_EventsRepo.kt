...
class fb_EventsRepo(context: Context) {
    ...
    fun createEvent(title:String, description:String, location:String, startTime:String, endTime:String, date:String,userid : String,groups : List<String> ,isSynced : Boolean, callback :(Boolean, String) -> Unit){
        val eventId = database.child("events").push().key ?: ""
        val event =
            Event(eventId, userid, title, description, location, startTime, endTime, date,groups,isSynced)
